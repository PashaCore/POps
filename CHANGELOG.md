...
### Added
- **Backend:** Agent-enrollment groundwork (first step toward authenticating agents). Migration `0002` adds `enroll_tokens` (one-time, lab-bound, expiring) and `agent_secrets` (per-device, stored as a SHA-256 hash keyed to the reconciled DNA identity, never plaintext), and superadmin endpoints create/list/revoke enrollment tokens (`POST` / `GET` / `DELETE /api/system/enroll-token[s]`). Consuming the token on first connect and enforcing authentication on `/ws/agent` and the agent HTTP endpoints come in the next steps.
