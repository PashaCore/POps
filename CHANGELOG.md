...
### Security
- **Agent:** The agent no longer talks to a server over plain `http://`. Over `ws://`, anyone on the same network could read the device secret and the enrollment token from the handshake and inject commands the agent runs as SYSTEM (`execute`, `set_secret`, `set_identity`); only `update_agent` is signed. With a non-loopback `http://` `ServerUrl` the agent now logs why and does not connect (the tray and watchdog keep working locally). The MSI refuses such a `SERVER_URL`, including one migrated from an older install. Plain `http://` stays allowed for `127.0.0.1` / `localhost`.
- **Agent:** The offline bypass code could be brute-forced. It is 6 hex characters (24 bits), and every logged-on user can write to the tray pipe without any attempt limit. After 5 wrong codes the bypass now locks for 15 minutes, doubling with each further lockout up to 24 hours. Codes are compared in constant time, and longer codes (a longer prefix of the same hash) are accepted, so the panel can move to longer codes without an agent change.
- **Agent:** A message on the tray pipe could declare any length, including negative values or gigabytes, and so exhaust the SYSTEM service's memory. Lengths outside 1 byte–32 MB now close the connection.
...
### Added
- **Backend:** The security audit log (`device_audit_logs`, which agents cannot write) is now **tamper-evident**: each entry stores a SHA-256 hash chained to the previous one (migration `0004`), serialized under an advisory lock, so editing or deleting any past entry breaks the chain. `GET /api/system/audit-verify` (superadmin) walks the chain and returns the first tampered id. Full write-prevention via a separate DB-owner role is the documented next step.
...
- **Agent:** Updates are installed and checked by `POpsUpdater`. It runs outside the install folder, from a copy that includes every dependency listed in its `deps.json`. It holds `C:\POpsData\update.lock`, backs up the install folder and runs `msiexec /i … /qn /norestart`. It then waits up to 90 seconds for the new version to write `C:\POpsData\health.json`, which the service now writes first thing on every start, before the WMI inventory. If the new version does not come up healthy, the updater installs the previous MSI with `POPS_ROLLBACK=1`, which removes the newer version inside the same Windows Installer transaction, so a failed rollback leaves the newer version installed and never leaves the machine without an agent. Each install keeps its package as `C:\POpsData\packages\installed.msi`, and the copy is checked by SHA-256, UpgradeCode, rollback support and installed ProductCode before use. Without a usable package, the updater restores the file backup. A 3010 exit (restart required) is reported as `pending_reboot` rather than rolled back. After every outcome the updater makes sure the `POpsAgent` service exists and runs; as a last resort it repairs from the package it has and logs `[KRİTİK]`. The outcome goes to `C:\POpsData\update-result.json`. While the lock is fresh (under 15 minutes), the watchdog restarts neither the service nor the tray.
- **Agent/Release:** Windows MSI installer, `POps-Agent-<version>-win-x64.msi` (WiX 5). The release workflow builds it and lists it in the signed manifest. It installs to `C:\Program Files\POps`, registers the `POpsAgent` service (LocalSystem, automatic start; Windows now restarts it on failure) and starts the tray for every user from `HKLM\...\Run`. The tray no longer writes its own `HKCU` Run entry. An Active Setup entry removes the one older versions left at each user's next sign-in, so an old tray cannot start next to the new one. `SERVER_URL`, `ENROLL_TOKEN`, `BYPASS_SECRET` and `PERSIST_DIR` are written by a custom action (secrets into `C:\POpsData\secure`, hidden from the MSI log), so an upgrade without properties keeps the existing settings. An upgrade runs in one transaction and rolls back to the installed version if it fails. Downgrades are refused, and the fourth version field is the CI run number. On a machine with a pre-MSI install, the MSI takes over the `POpsAgent` service and migrates the settings, reading `C:\POps` first because that is what released builds read. It then removes the old `C:\Program Files (x86)\POps` and `C:\POps` folders and leftovers such as `PashaCoreAgent.*` and `apply_update.bat`. The installer checks for the .NET 8 Desktop Runtime (x64) first. See `Installer/README.md`.
- **Agent:** Agent half of the new server endpoints. The Vision WebSocket sends `X-Agent-Secret` / `X-Enroll-Token` like `/ws/agent`. The inventory and policy-alert HTTP calls send `X-Agent-Id` + `X-Agent-Secret`. The updater's result (`update-result.json`) is reported once to the server as an `update_result` message (status, versions, detail, rollback, agent state and msiexec exit code). With `enforce_agent_auth` on, these calls keep working.
...
- **Agent:** The hardware inventory was built before the device identity was loaded, so it was always sent with an empty `hw_id`.
- **Agent:** A hanging WMI query could stall the service's start indefinitely. The start-up WMI queries now run in the background with a 15-second time-out each.
